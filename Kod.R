...
stochastic_triangle_forward_test_szybki <- function(data_paid, sigma_j, dev, sd, sim, Ultimate_Param_ReservingRisk,num_sim) {
  ...
  args <- lapply(1:sim, function(row) {
    list(seed = 202260011+num_sim, row = row, mu = mu, sigma = sigma, mm = mm, data_paid_copy = data_paid_copy, Ultimate_Param_ReservingRisk = Ultimate_Param_ReservingRisk)
  })
...
data_paid <- read.csv("reserv_data_MTPL_F.csv", sep = ";", dec = ",")[, -1]
