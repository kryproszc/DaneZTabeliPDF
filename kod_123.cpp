...
public:
  ...
  QuadTree& operator=(const QuadTree& other) {
    if (this != &other) { 
      ...
      nw = other.nw ? new QuadTree(*other.nw) : nullptr;
      ne = other.ne ? new QuadTree(*other.ne) : nullptr;
      sw = other.sw ? new QuadTree(*other.sw) : nullptr;
      se = other.se ? new QuadTree(*other.se) : nullptr;
    }
    return *this;
  }
  ...
  void showboundary()
  {
    std::cout << boundary.x << " " << boundary.width << " " << boundary.height << std::endl;
  }
  ~QuadTree() {
  ...
  bool insert(const Point& p) {
    ...
    if (p.x < boundary.x + boundary.width / 2) {
      if (p.y < boundary.y + boundary.height / 2) {
        return nw->insert(p);
      }
      else {
        return sw->insert(p);
  ...
  void subdivide() {
    ...
    Rect nwBoundary = { x, y, w, h };
    Rect neBoundary = { x + w, y, w, h };
    Rect swBoundary = { x, y + h, w, h };
    Rect seBoundary = { x + w, y + h, w, h };
...
std::vector<std::vector<double>> haversine_loop_cpp_vec(
    const std::vector<long double>& exposure_longitude,
    const std::vector<long double>& exposure_latitude,
    const std::vector<std::vector<long double>>& fire_spread_prob_vec,
    const std::vector<long double>& conditional_mean_trend_parameters,
    const std::vector<long double>& conditional_Cov,
    double radius,
    int n1,
    const std::vector<int>& exposure_insurance,
    const std::vector<int>& exposure_reassuramce,
    const std::vector<double>& exposure_sum_value,
    const std::vector<std::vector<double>>& wielkosc_pozaru,
    int woj, int mies)
  
{
  ...
  Rect searchArea = { left, top, size, size };
...
void simulateExponsure(size_t woj, int mies)
{
  int index_table = 0;
  if (exposure_longitude[woj][mies].size() > 0)
  {
    ...
    if (binom_fire > 0)
    {
      ...
      for (size_t budynki = 0; budynki < fire_sources_list.size(); budynki++)
      {
        ...
        spread_one_building = haversine_loop_cpp_vec(exposure_longitude[woj][mies], exposure_latitude[woj][mies],
                                                     fire_spread_prob_vec,
                                                     conditional_mean_trend_parameters,
                                                     conditional_Cov,16,
                                                     nr_budynku, exposure_insurance[woj][mies], exposure_reassurance[woj][mies],
                                                                                                                         exposure_sum_value[woj][mies], wielkosc_pozaru, woj, mies);
                                            ...
                                            int ilosc_ubezpieczycieli)
{
  ...
  for (int sim_num = 0; sim_num < sim; sim_num++)
  {
    for (size_t woj = 0; woj < 17; woj++)
    {
      ...
      {
        pool.enqueue([woj, mies] {
          simulateExponsure(woj, mies);
        });
      }
    }
    pool.wait_for_tasks();
...
int main()
{
  ...
  for (int woj = 0; woj < numRegions; ++woj)
  {
    ...
    {
      int n_num = generateRandomInts(1, 80000, 80000)[0];  
      exponsure_latitude[woj][month] = generateRandomLongDoubles(n_num, 0,180);
      exponsure_longitude[woj][month] = generateRandomLongDoubles(n_num, 0,180);
      ...
      for(long int ic = 0; ic < n_num ; ic++)
        getQuadTree(quadtree,woj,month).insert({
          exponsure_latitude[woj][month][ic],
                                        exponsure_longitude[woj][month][ic],
                                                                       exponsure_insurance[woj][month][ic],
                                                                                                      exponsure_reassurance[woj][month][ic],
                                                                                                                                       exponsure_sum_value[woj][month][ic]
        });
