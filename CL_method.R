...
line_buissnes="MTPL_I"

setwd("C:/Users/szczkr/Desktop/Symulacje")
result_out <-c()
options(digits = 10)
wsp <- read_xlsx(path =paste0("wspol_",line_buissnes,".xlsx"))
dev <- as.numeric(wsp[1, 1:59])
sigma_j <- as.numeric(wsp[2, 1:59])
sd <- as.numeric(wsp[3, 1:59])
data_paid <- read.csv(paste0("reserv_data_",line_buissnes,".csv"), sep = ";", dec = ",")[, -1]
...
stochastic_triangle_forward_test_szybki <- function(data_paid, sigma_j, dev, sd, sim, Ultimate_Param_ReservingRisk, num_sim, batch_size = 1000, result_file = "results.csv") {
  ...
  # Wywo³anie random_stochastic_parameters raz
  ...
  for (i in seq(1, sim, by = batch_size)) {
    ...
    args <- lapply(1:current_batch_size, function(batch_row) {
      global_row <- i + batch_row - 1  # Przesuniêcie indeksu, aby uwzglêdniæ numer partii
    ...
    # Zapis wyników do pliku po ka¿dej partii
    write.table(batch_results, file = result_file, sep = ",", col.names = FALSE, row.names = FALSE, append = TRUE)
    
    # Debugowanie
    cat("Zamkniêto klaster dla partii od", i, "do", i + current_batch_size - 1, "\n")
  }
  
  results
}
# Przyk³ad wywo³ania
result <- stochastic_triangle_forward_test_szybki(data_paid, sigma_j, dev, sd, 100000, 10329955580, 0)
